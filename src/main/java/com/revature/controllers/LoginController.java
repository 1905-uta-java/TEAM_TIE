...
public class LoginController {
	...
	public ResponseEntity<String> listTrainers(@RequestBody String[] creds) {
		...
		if(creds.length != 2)
			return new ResponseEntity<>("Failed to login.", responseHeaders, HttpStatus.FORBIDDEN);
		...
		if(lt != null) {
			if(lt.getPass().equals(pass)) {
				...
				return new ResponseEntity<>("Successfully logged in.", responseHeaders, HttpStatus.OK);
			}
			else {
				return new ResponseEntity<>("Failed to log in.", responseHeaders, HttpStatus.FORBIDDEN);
			}
		}
		else {
			return new ResponseEntity<>("Failed to login.", responseHeaders, HttpStatus.FORBIDDEN);
